  ...
  this._socket.addEventListener("message", function (data, flags) {
    util.setZeroTimeout(function () {
      ...
      try {
        ...
        return self.emit(
          "error",
          new Error("Received unparsable message: " + ex)
        );
      }
      ...
      if (data.length != 3)
        return self.emit(
          "error",
          new Error("Received message with wrong part count: " + data.length)
        );
      if ("number" != typeof data[0])
        return self.emit(
          "error",
          new Error("Received message with non-number type: " + data[0])
        );
      ...
      switch (data[0]) {
        ...
        case 2:
          ...
          if (binaryStream) {
            ...
            self.emit(
              "error",
              new Error(
                "Received `data` message for unknown stream: " + streamId
              )
            );
          }
          break;
        case 3:
          ...
          if (binaryStream) {
            ...
            self.emit(
              "error",
              new Error(
                "Received `pause` message for unknown stream: " + streamId
              )
            );
          }
          break;
        case 4:
          ...
          if (binaryStream) {
            ...
            self.emit(
              "error",
              new Error(
                "Received `resume` message for unknown stream: " + streamId
              )
            );
          }
          break;
        case 5:
          ...
          if (binaryStream) {
            ...
            self.emit(
              "error",
              new Error(
                "Received `end` message for unknown stream: " + streamId
              )
            );
          }
          break;
        case 6:
          ...
          if (binaryStream) {
            ...
            self.emit(
              "error",
              new Error(
                "Received `close` message for unknown stream: " + streamId
              )
            );
          }
          break;
        default:
          self.emit(
            "error",
            new Error("Unrecognized message type received: " + data[0])
          );
      }
    });
  });
...
BinaryClient.prototype.send = function (data, meta) {
  ...
  if (data instanceof Stream) {
    ...
    if (Buffer.isBuffer(data)) {
      new BufferReadStream(data, { chunkSize: this._options.chunkSize }).pipe(
        stream
      );
    } else {
      stream.write(data);
    ...
    if (data.constructor == Blob || data.constructor == File) {
      new BlobReadStream(data, { chunkSize: this._options.chunkSize }).pipe(
        stream
      );
      ...
      new BlobReadStream(blob, { chunkSize: this._options.chunkSize }).pipe(
        stream
      );
